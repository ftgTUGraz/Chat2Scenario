...
class TrackVisualizer(object):
    def __init__(self, config: dict, tracks: List[dict], tracks_meta: List[dict], recording_meta: dict):
        ...
        if hasattr(toolbar, 'toolitems'):
            # Get toolbar button
            toolitems = [tool for tool in toolbar.toolitems if tool[0] not in ['Subplots', 'Save', 'Customize', 'Forward', 'Back']]
            # Set a new toolbar button
            toolbar.toolitems = toolitems
        else:
            for x in toolbar.actions():
                if x.text() in ['Subplots', 'Save', 'Customize', 'Forward', 'Back']:
                    toolbar.removeAction(x)

        # Set the window title
        plt.get_current_fig_manager().set_window_title("Tracks Visualizer - Dataset {}, Recording {}".format(self.dataset, self.recording_name))
