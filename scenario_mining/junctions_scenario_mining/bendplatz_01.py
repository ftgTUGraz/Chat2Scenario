...
class BendplatzProcessor:
    ...
    def find_two_nearest_endpoints_per_road(x_endpoints, y_endpoints, road_ids):
        ...
        unique_road_ids = set(road_ids)  # Unique road IDs
        sorted_road_ids = sorted(unique_road_ids)  # Sorted road IDs
        nearest_endpoints_per_road = {str(road_id): [] for road_id in sorted_road_ids}

        for i in range(len(x_endpoints)):
            current_road_id = str(road_ids[i])  # Current road ID as string
    ...
    def process_opendrive_file(self, filepath):
                            ...
                            for lane in laneSection.findall('.//lane'):
                                ...
                                if lane_type == "driving" or lane_type == "parking":
                                    ...
                                    for widthElement in lane.findall('.//width'):
                                        ...
                                        lane_widths = [BendplatzProcessor.calculate_width(point + s, a, b, c, d) for point in points]
        ...
        for road_id, lanes in previous_lane_coords.items():
            ...
            for i in range(len(lane_ids) - 1):
                lane_id = lane_ids[i]
                lane_id_call = i + 1  # Artificially set the minimum lane number as 1
        ...
        two_nearest_endpoints_per_road, polygan_jonctions = BendplatzProcessor.find_two_nearest_endpoints_per_road(
            x_endpoints, y_endpoints, road_endpoint_ids)

        # Create connections
        connections = BendplatzProcessor.connect_nearest_endpoints(two_nearest_endpoints_per_road, road_endpoint_ids)
        ...
        patch = PathPatch(path, facecolor='gray', edgecolor='none', alpha=0.5)
        plt.gca().add_patch(patch)
        self.road_lane_paths[100] = {}
        self.road_lane_paths[100][1] = path
    ...
    def find_lane_by_coordinates(self, x, y):
        ...
        return -1, 1  # Return (-1, 1) if no matching lane is found, typically indicating the point is within a junction.

    def update_tracks_with_lane_info(self, csv_path):
        """
        Update tracks with road and lane information based on coordinates.

        Parameters:
        ----------
        csv_path (str): Path to the CSV file containing track data.
    ...
    def extract_road_id(road_lane):
        """
        Extract the road ID from a combined 'road.lane' string.
    ...
    def determine_turn_type(path):
        """
        Determine the type of turn based on road IDs before and after a specific segment.
    ...
    def process_tracks(df):
        ...
        grouped['turn_type'] = grouped['unique_path'].apply(BendplatzProcessor.determine_turn_type)
